...
echo -e "${BLUE}>>> BCMR 安装程序 (CN Proxy)${NC}"
...
# Define URLs
# Using ghfast.top proxy for GitHub releases
DOWNLOAD_URL="https://ghfast.top/https://github.com/Bengerthelorf/bcmr/releases/latest/download/${ASSET_NAME}"
...
if [ -z "$BINARY_PATH" ]; then
    echo -e "${RED}错误: 在压缩包中未找到 'bcmr' 二进制文件。${NC}"
    exit 1
fi
...
echo -e "运行 ${BLUE}bcmr --version${NC} 进行验证。"
